...
## Markdown 엔진의 호환성 문제(내가 잘못했음...)로 github 페이지에서 코드블록이 제대로 안먹히네요. 조만간 수정하겠습니다.
...
2. `docker service inspect --pretty <SERVICE-ID>`를 실행해서 service의 상태를 읽기 쉽게 표시. helloworld의 경우.
  ...
  hello world 서비스가 workder2 에서 실행중이지만 manager noded에서 실행될 수도 있음. default로 manager node도 worker node 처럼 task를 실행할 수 있음. `DESIRED STATE` 와 `LAST STATE`도 표시해서 지정한 수만큼의 task가 실행중인지 확인가능.
...
9. node를 inspect해서 결과 확인
  ...
  - 어떤 Active node에 있던 task가 fail 했을 대
